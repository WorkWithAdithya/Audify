...
export const getSongWithAuth = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    try {
      const purchaseCheck = await axios.get(
        `${process.env.USER_SERVICE_URL}/api/v1/user/purchase/check/${songId}`,
        {
          headers: {
...
export const streamSong = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    const songId = req.params.id;
    const userId = req.user?._id;
...
export const getMyPurchasedSongs = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    const userId = req.user?._id;
    ...
    try {
      ...
      const purchasedResponse = await axios.get(
        `${process.env.USER_SERVICE_URL}/api/v1/user/purchases`,
        {
          headers: {
            token: req.headers.token as string,
          },
