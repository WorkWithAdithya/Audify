...
const PlayList = () => {
     ...
     useEffect (()=>{
        if(songs && user?.playlist){
            const filteredSongs = songs.filter((songs)=>
                user.playlist.includes(songs.id.toString())
            )
  ...
  return (
        ...
        <>
        {
            ...
            <div className="mt-6 sm:mt-8 md:mt-10 flex gap-4 sm:gap-6 md:gap-8 flex-col md:flex-row md:items-center">
              ...
              <div className="relative group">
                ...
                {/* Favorite badge */}
                <div className="absolute top-2 right-2 bg-gradient-to-r from-pink-500 to-purple-600 text-white text-xs font-bold px-2 py-1 rounded-full shadow-lg">
                  ❤️ Favs
                </div>
              </div>
            ...
            <div className="flex flex-col gap-2 sm:gap-3">
                ...
                <h4 className="text-sm sm:text-base md:text-lg text-gray-400 leading-relaxed flex items-center gap-2">
                  <span className="text-pink-400">🎵</span> Your favorite songs
                </h4>
                <p className="text-xs sm:text-sm text-gray-500 mt-1">
                  {myPlayList.length} {myPlayList.length === 1 ? 'track' : 'tracks'}
                </p>
    ...
    {
        myPlayList && myPlayList.length > 0 ? myPlayList.map((song, index) => {
            return (
                <div 
            ...
            <div className="flex flex-col items-center justify-center py-16 sm:py-20 md:py-24">
                ...
                <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-400 mb-2">No songs in your playlist yet</h3>
                <p className="text-sm sm:text-base text-gray-500">Start adding your favorite tracks!</p>
            </div>
        )
    }
