...
const Player = () => {
  ...
  return (
    <div>
      {
        song && <div className="h-[10%] bg-gradient-to-br from-black via-gray-900 to-black border-t border-purple-500/20 flex justify-between items-center text-white px-3 sm:px-4 md:px-6 py-2 sm:py-3 backdrop-blur-xl">
        <div className="lg:flex items-center gap-3 sm:gap-4">

          {/* Rotating Vinyl Disk Animation */}
          <div className="relative w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16">
            {/* Outer vinyl disk */}
            <div className={`absolute inset-0 rounded-full bg-gradient-to-br from-gray-900 via-black to-gray-800 border-4 border-purple-500/30 shadow-lg shadow-purple-500/20 ${isPlaying ? 'animate-spin' : ''}`} style={{animationDuration: '3s'}}>
            ...
            {/* Center image that rotates */}
            <div className={`absolute inset-0 flex items-center justify-center ${isPlaying ? 'animate-spin' : ''}`} style={{animationDuration: '3s'}}>
              <img 
          ...
          <div className="hidden md:block">
            <p className="font-bold text-sm lg:text-base bg-gradient-to-r from-purple-400 via-pink-400 to-cyan-400 bg-clip-text text-transparent">{song.title}
              <p className="text-xs lg:text-sm text-gray-400 font-normal mt-0.5">{song.description?.slice(0,30)}...</p>
            </p>
          </div>
          ...
          <div className="flex flex-col items-center gap-1 sm:gap-2 m-auto">
            ...
            <div className="w-full items-center flex font-thin">
              <input 
                ...
                style={{
                  background: `linear-gradient(to right, rgb(168, 85, 247) 0%, rgb(168, 85, 247) ${(progress / duration) * 100}%, rgb(55, 65, 81) ${(progress / duration) * 100}%, rgb(55, 65, 81) 100%)`
                }}
              />
            </div>
            
            <div className="flex justify-center items-center gap-3 sm:gap-4 md:gap-6">
                <span className="cursor-pointer text-purple-400 hover:text-cyan-400 transition-all duration-300 hover:scale-125 text-lg sm:text-xl md:text-2xl" onClick={prevSong}>
                  <GrChapterPrevious/>
                </span>

                <button className="bg-gradient-to-r from-purple-500 via-pink-500 to-cyan-500 hover:from-purple-600 hover:via-pink-600 hover:to-cyan-600 text-white rounded-full p-2 sm:p-3 transition-all duration-300 hover:scale-110 shadow-lg shadow-purple-500/50 hover:shadow-xl hover:shadow-cyan-500/50" onClick={handlePlayPause}>
                  {isPlaying? <FaPause className="text-sm sm:text-base"/> : <FaPlay className="text-sm sm:text-base"/>}
                </button>

               <span className="cursor-pointer text-purple-400 hover:text-cyan-400 transition-all duration-300 hover:scale-125 text-lg sm:text-xl md:text-2xl" onClick={nextSong}>
                <GrChapterNext/>
               </span>
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs sm:text-sm text-purple-400 hidden sm:block">🔊</span>
            <input 
              type="range" 
