...
const getRazorpayKeyId = () => {
    ...
    if (!keyId)
        throw new Error("RAZORPAY_KEY_ID not defined");
    return keyId;
};
const getRazorpayKeySecret = () => {
    const keySecret = process.env.RAZORPAY_KEY_SECRET;
    if (!keySecret)
        throw new Error("RAZORPAY_KEY_SECRET not defined");
    return keySecret;
};
const getSongServiceUrl = () => {
    const url = process.env.SONG_SERVICE_URL;
    if (!url)
        throw new Error("SONG_SERVICE_URL not defined");
    return url;
};
...
export const createBulkOrder = TryCatch(async (req, res) => {
    ...
    try {
      ...
      `;
        if (alreadyPurchased.length > 0) {
            const purchasedIds = alreadyPurchased.map((p) => p.song_id);
        ...
        const songsData = await Promise.all(songIds.map(async (songId) => {
...
export const verifyBulkPayment = TryCatch(async (req, res) => {
    ...
    try {
        for (const purchase of purchases) {
            await axios.post(`${getUserServiceUrl()}/api/v1/user/purchase/add`, {
...
export const createOrder = TryCatch(async (req, res) => {
    ...
    try {
        const songResponse = await axios.get(`${getSongServiceUrl()}/api/v1/song/${songId}`);
...
export const getPurchaseHistory = TryCatch(async (req, res) => {
    const userId = req.user?._id;
    if (!userId) {
        res.status(401).json({ message: "User not authenticated" });
        return;
    ...
    const purchasesWithSongs = await Promise.all(purchases.map(async (purchase) => {
        try {
            const songResponse = await axios.get(`${getSongServiceUrl()}/api/v1/song/${purchase.song_id}`);
            return {
                ...purchase,
...
export const getPurchaseDetails = TryCatch(async (req, res) => {
    const userId = req.user?._id;
    ...
    // Fetch song details
    try {
        const songResponse = await axios.get(`${getSongServiceUrl()}/api/v1/song/${purchaseData.song_id}`);
        res.json({
            purchase: {
                ...purchaseData,
                song: songResponse.data,
...
export const getAllTransactions = TryCatch(async (req, res) => {
    ...
    if (status) {
      ...
      `;
        ...
        SELECT COUNT(*) FROM transactions WHERE status = ${status}
      `;
    }
    else {
        transactions = await sql `
...
export const getRevenueStats = TryCatch(async (req, res) => {
    if (req.user?.role !== "admin") {
        res.status(403).json({ message: "Admin access required" });
        return;
    }
